...
if __name__ == '__main__':
    ...
    rf_sim_matrix = relevance_feedback(vec_docs, vec_queries, sim_matrix, gt)
    print("The time difference is :", timeit.default_timer() - start)
    if args.verbose:
    ...
    start = timeit.default_timer()
    rf_sim_matrix = relevance_feedback_exp(vec_docs, vec_queries, sim_matrix, gt, tfidf_model)
    print("The time difference is :", timeit.default_timer() - start)
    if args.verbose:
        for i in range(rf_sim_matrix.shape[1]):
            ranked_documents = np.argsort(-rf_sim_matrix[:, i])
